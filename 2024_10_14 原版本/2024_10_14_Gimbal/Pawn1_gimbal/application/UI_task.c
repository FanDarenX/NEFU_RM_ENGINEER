...
void draw_cha_Init(void);//初始化UI。初始化字符串
...
void Figure_Graphic(graphic_data_struct_t* graphic,//最终要发出去的数组的数据段内容
									...
									uint32_t end_x,
									uint32_t end_y);
void Float_Graphic(Float_data_struct_t* graphic,//最终要发出去的数组中的数据段内容
...
static void Float_supercapacitor_update();//超级电容条绘制
...
ext_float_two_data_t super_data; //绘制超级电容电压
/*结构体实例化END*/
...
uint32_t global_sight_bead_x = 960,global_sight_bead_y = 720,global_supercapacitor_point = 40;//[0,100]
char first_line[30]  = {"Cover"};//弹仓盖
char second_line[30] = {"Chais"};//底盘状态以及小陀螺
char third_line[30]  = {"Fire"};//发射机构状态
char exter_fouth_line[30] = {"Protect"};
char fourth_line[30] = {"Mode"};//模式
char fifth_line[30]  = {"CRAZY"};
char sixth_line[30]  = {"CALM"};
char seventh_line[30] = {"SIT_DOWN"};//蹲下
char eighth_line[30]  = {"STAND_UP"};//起立
char nineth_line[30]  = {"NORMAL"};  //普通
char empty_line[30] = {"                           "};
...
void UI_task(void const *pvParameters)
{
    ...
    if(i%Client_circle == 0)
		...
		{//更新第三层图层，放置电容状态
			...
			Float_supercapacitor_update();
			float_flag=MODIFY;
		}
		else if(i%Client_circle == 7)
		{//更新第四层图层，自瞄框
			autoshoot_allow();
			auto_sope_flag = MODIFY;
		}
		else if(i%Client_circle == 9)
		{//更新第四层图层，放置战略建议，待完善
//			guard_control();
//			guard_flag = MODIFY;
		}
...
//		else if(i%Client_circle == 10)
//		{
////			Client_aim_update();
////			update_aim_flag = ADD;
//		}
		else if(i%Client_circle == 8)
		{
			_lowlong_aim_();
			lowlong_flag=ADD;
		}
...
void UI_Init()
{
	  static uint32_t i;
		flag_fresh();	
		if(RM_Referee.robot_state.robot_id==3)			 {Recieve_id = 0x103;Guard_id = 7;}
		else if(RM_Referee.robot_state.robot_id==4)  {Recieve_id = 0x104;Guard_id = 7;}
		else if(RM_Referee.robot_state.robot_id==5)  {Recieve_id = 0x105;Guard_id = 7;}
		else if(RM_Referee.robot_state.robot_id==103){Recieve_id = 0x167;Guard_id = 107;}
		else if(RM_Referee.robot_state.robot_id==104){Recieve_id = 0x168;Guard_id = 107;}
		else if(RM_Referee.robot_state.robot_id==105){Recieve_id = 0x169;Guard_id = 107;}
			
		draw_cha_Init();//不用一直更新，但是无法判断什么时候进入客户端所以需要轮询,可以操作手key控制	
		//update_figure_flag = ADD;
		vTaskDelay(50);
		Client_graphic_Info_update();
		update_figure_flag = ADD;
...
{
	/*****************底盘运动**********************/
	if(!switch_is_mid(gimbal_control.gimbal_rc_ctrl->rc.s[GIMBAL_MODE_CHANNEL]))//gimbal_behaviour == GIMBAL_ZERO_FORCE
	{chassis_judge=0;}//红
	else
	{
		if(sup_cap.tl_flag==1)
			chassis_judge=2;//橙
		else if( sup_cap.side_flag==1)
			chassis_judge=3;//蓝
		else 
			chassis_judge=1;//绿
	}
	...
	/*******************超级电容***********************/
	global_supercapacitor_remain=(super_cap.Value_Cap-20)/6;
}
void draw_cha_Init(void)//写文字
{ 
		//帧头
...
void Float_supercapacitor_update()//超级电容条绘制
{
    //帧头
    super_data.txFrameHeader.sof = JUDGE_FRAME_HEADER;
    super_data.txFrameHeader.data_length = sizeof(ext_client_data_header_t) + sizeof(graphic_data_struct_t);
    super_data.txFrameHeader.seq = 0;//包序号
    memcpy(CliendTxBuffer,&super_data.txFrameHeader,sizeof(std_frame_header_t));
    append_CRC8_check_sum(CliendTxBuffer, sizeof(std_frame_header_t));//头校验
    //命令码
    super_data.CmdID = ID_robot_interactive_header_data;
    //数据段头结构
    super_data.dataFrameHeader.data_cmd_id = INTERACT_ID_draw_two_graphic;
    super_data.dataFrameHeader.send_ID     =  RM_Referee.robot_state.robot_id;
    super_data.dataFrameHeader.receiver_ID = Recieve_id;
    //数据段
    //supercapacitor_figure(global_supercapacitor_remain,global_supercapacitor_point);

    Float_Graphic(&super_data.clientData[0],"SP1",float_flag,0,1,WHITE,0,0,10,AIM_X+100,AIM_Y+100,super_cap.Value_Cap);
    memcpy(CliendTxBuffer+LEN_FRAME_HEAD, (uint8_t*)&super_data.CmdID, LEN_CMD_ID+super_data.txFrameHeader.data_length);//加上命令码长度2

    //帧尾
    append_CRC16_check_sum(CliendTxBuffer,sizeof(super_data));

    Client_Sent_String(CliendTxBuffer, sizeof(super_data));
}
...
{ 
	int8_t X=-10,Y=65;uint32_t lightline_position_x=0,lightline_position_y=0;
	X=8;Y=45;
	...
	lightline_position_x=AIM_X+500+(int32_t)(arm_cos_f32(-gimbal_control.gimbal_yaw_motor.relative_angle)*70);
	lightline_position_y=AIM_Y+250+(int32_t)(arm_sin_f32(-gimbal_control.gimbal_yaw_motor.relative_angle)*70);
                   ...
                   float number)
					{
							for(char i=0; i<3; i++)
									graphic->graphic_name[i] = name[i];	//字符索引
							graphic->operate_tpye = operate_tpye; //图层操作
							graphic->graphic_tpye = FLOAT;         //Float型
							graphic->layer        = layer;//都在第一层
							graphic->color        = color;//变色
							graphic->start_angle  = start_angle;
							graphic->end_angle    = end_angle;
...
{
	state_first_graphic++;
	if(state_first_graphic == 1)//不知道什么时候进入客户端所以要不断更新
	{//autofire自瞄状态和是否可以射击
...
//	{//模式
//		Char_Graphic(&tx_client_char.clientData,"CL4",char_flag,0,WHITE,15,strlen(exter_fouth_line),1,(500),(1080*6/12),exter_fouth_line);
//	}	
//	else if(state_first_graphic == 15)
//	{//firction发射机构状态
//		Char_Graphic(&tx_client_char.clientData,"CL5",char_flag,0,WHITE,15,strlen(third_line),1,(500),(1080*5/12),third_line);
//	}		
	else if(state_first_graphic == 4)
	{//狂暴模式
		if(gimbal_control.shoot_mood)
			Char_Graphic(&tx_client_char.clientData,"CL4",char_flag,0,PINK,20,strlen(fifth_line),2,(1000),(1080*9/12),fifth_line);
		else 
			Char_Graphic(&tx_client_char.clientData,"CL4",char_flag,0,GREEN,20,strlen(sixth_line),2,(1000),(1080*9/12),sixth_line);
	}
	else if(state_first_graphic == 5)
	{//当前腿长
		if(sup_cap.sit_flag==1)
			Char_Graphic(&tx_client_char.clientData,"CL5",char_flag,0,CYAN_BLUE,20,strlen(seventh_line),2,(800),(1080*9/12),seventh_line);
		else if(sup_cap.stand_flag==1)
			Char_Graphic(&tx_client_char.clientData,"CL5",char_flag,0,PINK,20,strlen(eighth_line),2,(800),(1080*9/12),eighth_line);	
		else 
			Char_Graphic(&tx_client_char.clientData,"CL5",char_flag,0,GREEN,20,strlen(nineth_line),2,(800),(1080*9/12),nineth_line);
	}
	if(state_first_graphic>=6)state_first_graphic=0;		
}
static void supercapacitor_figure(fp32 remain_energy,uint32_t turning_point)//剩余超级电容（单位百分比），低于某百分比变红色
{
...
static void Mode(void)
{
	if(sup_cap.protect_flag)
	{
		Figure_Graphic(&tx_client_graphic_figure.clientData[3],"GL4",update_figure_flag,CIRCLE,1,FUCHSIA,0,0,10,  600,1080*6/12, 25,0,0);
	}
	else
	{
