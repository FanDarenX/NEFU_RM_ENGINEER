...
{		
	...
	{	
		if((barrel_choice_flag==1&&RM_Referee.power_heat_data_t.shooter_heat0 < RM_Referee.robot_state.shooter_id1_17mm_cooling_limit)||
							 (barrel_choice_flag==2&&RM_Referee.power_heat_data_t.shooter_heat1 < RM_Referee.robot_state.shooter_id1_17mm_cooling_limit))
		 {
	...
	{
		if(rc_ctrl.key.v &KEY_PRESSED_OFFSET_R && change_flag==0)
		{
			change_num=0;change_flag=1;
			fire_speed=7100;
		}
		else if(test_fire_flag && change_flag==0 && !test_flag)
		{
			change_num=0;change_flag=1,test_flag=1;
			fire_speed=7100;
		}		
		else if((rc_ctrl.key.v &KEY_PRESSED_OFFSET_R) && change_flag==1)
		{
			change_num=0;change_flag=0;
			fire_speed=7100;
		}
		else if(!test_fire_flag && change_flag==1 && test_flag)
		{
			change_num=0;change_flag=0,test_flag=0;
			fire_speed=7100;
		}		
	}
	
	if((switch_is_mid(rc_ctrl.rc.s[1])==1||switch_is_up(rc_ctrl.rc.s[1])==1) && change_flag==1)
	{
		shoot_control.shoot_mode = SHOOT_READY;
	  shoot_control.fire_speed1_set =fire_speed;
	  shoot_control.fire_speed2_set =-fire_speed;
	}
	else if(switch_is_mid(rc_ctrl.rc.s[1])==0 || change_flag==0)
	{ 
		shoot_control.shoot_mode = SHOOT_STOP;
		shoot_control.fire_speed1_set=0;
		shoot_control.fire_speed2_set=0;
  ...
  if((switch_is_mid(rc_ctrl.rc.s[1]))||(switch_is_up(rc_ctrl.rc.s[1]))||test_fire_flag)//当拨杆位于中间or test
	{
			if (shoot_heat_control()==0)
			{num=0;fire_ready_flag=0;}
			else if(shoot_heat_control()==1)
			{
				if(num<300)num++;//热量低的时候低射频
				else
				{
					if((fire_ready_flag==0)&&(rc_ctrl.mouse.press_l==1||test_fire_flag))
					{ fire_ready_flag=1;num=0;}
				}  
			}
			else if(shoot_heat_control()==2) 
			{ 
				if(num<150)num++;//热量足的时候高射频
				else
				{ 
					if((fire_ready_flag==0)&&(rc_ctrl.mouse.press_l==1||test_fire_flag))
					{ fire_ready_flag=1;num=0;}
		...
		{ 
			...
			{
			  if(gimbal_control.shoot_mood) //连射，明天试试
					shoot_control.set_angle-=1.0f*(shoot_control.shooter_heat_limit-RM_Referee.power_heat_data_t.shooter_heat0);	
			  else 
					shoot_control.set_angle-=45.0f;	
				fire_ready_flag=2;
			}
		}
	}
...
static void shoot_feedback_update(void)
{
	...
	if(switch_is_mid(rc_ctrl.rc.s[1])||test_fire_flag)
	{
	...
	if(fire_temp_last==0 && fire_temp==1)
		shoot_control.set_angle = shoot_control.angle;
	if(shoot_control.set_angle > shoot_control.angle)
		shoot_control.set_angle = shoot_control.angle;
...
{ 	
	  ...
	  magazine_cover_control();
		...
		if(barrel_ready_flag==0)
		{
			...
			else if(switch_is_mid(rc_ctrl.rc.s[1])||(test_fire_flag==1))
			{
...
void magazine_cover_control()
{
	if(mc_num<=200)
		mc_num++;
	else if(mc_num>200&&(mc_flag==1 && rc_ctrl.key.v & KEY_PRESSED_OFFSET_Z))
	{
		__HAL_TIM_SET_COMPARE(&htim8,TIM_CHANNEL_3,750);//750 1900
		mc_num=0;
		mc_flag=0;
	}
	else if(mc_num>200&&(mc_flag==0 && rc_ctrl.key.v & KEY_PRESSED_OFFSET_Z))
	{
		__HAL_TIM_SET_COMPARE(&htim8,TIM_CHANNEL_3,1850);//750 1900
		mc_flag=1;
		mc_num=0;
	}	
}	
