...
#define   WAVEFORM_COUNT 4        //有几路波形 会发送前x路波形
...
{
    //data
    waveform[USERDATA_1]   = (short)(shoot_control.fire_speed1);//(short)(gimbal_control.gimbal_yaw_motor.given_current);
    waveform[USERDATA_2]   = (short)(shoot_control.fire_speed2);//(gimbal_control.gimbal_yaw_motor.absolute_angle_set1*1000);//(gimbal_control.gimbal_yaw_motor.absolute_angle*500);
    waveform[USERDATA_3]   = 0;
    waveform[USERDATA_4]   = 1000;
...
//              _buff[10] = 10 & 0xFF;
	...
	//清除数据
    ano_fun = 0;ano_fun_1 = 0;ano_read_val_num = 0;
	WAVESHOW_FLAG = 0;
}
