...
typedef struct {  // 以 4个字节一组排列 ，不然编译器会凑整
    ...
    A1_COMData32  Modify;     // 电机参数修改 的数据 
    //实际给FOC的指令力矩为：
    //K_P*delta_Pos + K_W*delta_W + T
    q15_t     Tor;      // 期望关节的输出力矩（电机本身的力矩）x256, 7 + 8 描述
    q15_t     Spd;      // 期望关节速度 （电机本身的速度） x128,       8 + 7描述	
    ...
    uint8_t A1_LowHzMotorCmdIndex;     // 电机低频率控制命令的索引, 0-7, 分别代表LowHzMotorCmd中的8个字节
    uint8_t A1_LowHzMotorCmdByte;      // 电机低频率控制命令的字节
	
     A1_COMData32  Res[1];    // 通讯 保留字节  用于实现别的一些通讯内容
	
}A1_MasterComdV3;   // 加上数据包的包头 和CRC 34字节

typedef struct {
	// 定义 电机控制命令数据包	
    A1_COMHead head;    
    A1_MasterComdV3 Mdata;
    A1_COMData32 CRCdata;
}A1_MasterComdDataV3;//返回数据
...
typedef struct {  // 以 4个字节一组排列 ，不然编译器会凑整
    ...
    A1_COMData32  Read;     // 读取的当前 电机 的控制数据 
...
}A1_ServoComdV3;  // 加上数据包的包头 和CRC 78字节（4+70+4）

typedef struct {
    // 定义 电机控制命令数据包	
    A1_COMHead        head;
    A1_ServoComdV3      Mdata;

    A1_COMData32    CRCdata;

}A1_ServoComdDataV3;	//发送数据
