...
//  HAL_UART_Receive_DMA(&huart8, usart8_rx, 41);
...
void MX_USART6_UART_Init(void)
{
  ...
  //	HAL_UART_Receive_DMA(&huart6, usart8_rx,17);
 ...
 * @brief 串口初始化函�?
 * @author RM、Skr
 * @param *rx1_buf�?*rx2_buf 缓冲接收数组的指�?
 * @param 缓冲数组的大�?
 */
void uart8_init(uint8_t *rx1_buf, uint8_t *rx2_buf, uint16_t dma_buf_num)
{
    ...
    hdma_uart8_tx.Instance->PAR = (uint32_t) & (USART6->DR);
}
/* USER CODE END 1 */
